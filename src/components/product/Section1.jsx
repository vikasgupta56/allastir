  ...
  return (
    <div className='relative w-full h-screen flex flex-col items-center justify-center text-[15vw] font-bold uppercase'>
      <div className='overflow-hidden relative z-[10]'>
        <h1 className='leading-none text-center about-span'> {"our".split("").map((l, i) => <span style={{transform:"translateY(100%)"}} className='inline-block leading-tight' key={i}>{l}</span>)}</h1>
      </div>
      <div className='overflow-hidden relative z-[10]'>
        <h1 className='leading-none text-center about-span'> {"products".split("").map((l, i) => <span style={{transform:"translateY(100%)"}} className='inline-block leading-tight' key={i}>{l}</span>)}</h1>
      </div>
